...
export function validateOptions<P, X>(
    options: ComponentOptionsType<P, X> | null | undefined
) {
    // eslint-ignore-line
    if (!options) {
    ...
    if (
        !options.tag ||
        !options.tag.match(/^([a-z0-9][a-z0-9-]*)+[a-z0-9]+$/)
    ) {
    ...
    if (options.dimensions) {
        if (
            options.dimensions &&
            !isPx(options.dimensions.width) &&
            !isPerc(options.dimensions.width)
        ) {
            throw new Error(
        ...
        if (
            options.dimensions &&
            !isPx(options.dimensions.height) &&
            !isPerc(options.dimensions.height)
        ) {
            throw new Error(
        ...
        if (
            ...
            throw new Error(
                `Unsupported context type: ${
                    options.defaultContext || 'unknown'
                }`
            );
        }
    }
