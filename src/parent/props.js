...
export function extendProps<P, X>(propsDef : PropsDefinitionType<P, X>, existingProps : PropsType<P>, inputProps : PropsInputType<P>, helpers : ParentHelpers<P>, container : HTMLElement | void) {
    ...
    const nextProps = { ...inputProps };

    // $FlowFixMe
    eachProp(nextProps, propsDef, (key, propDef, value) => {
        if (!propDef) {
            nextProps[key] = value;
            return;
        }
        ...
        if (alias && !isDefined(value) && isDefined(nextProps[alias])) {
            value = nextProps[alias];
        }

        if (propDef.value) {
            value = propDef.value({ props: { ...nextProps }, state, close, focus, event, onError, container });
        }

        if (propDef.default && !isDefined(value) && !isDefined(nextProps[key])) {
            value = propDef.default({ props: { ...nextProps }, state, close, focus, event, onError, container });
        }
        ...
        if (isDefined(value)) {
            ...
            if (propDef.required !== false && !isDefined(nextProps[key])) {
        ...
        if (__DEBUG__ && isDefined(value) && propDef.validate) {
            // $FlowFixMe
            propDef.validate({ value, props: nextProps });
        }
    ...
    eachProp(nextProps, propsDef, (key, propDef, value) => {
        ...
        if (isDefined(value) && propDef.decorate) {
            // $FlowFixMe
            const decoratedValue = propDef.decorate({ value, props: { ...nextProps }, state, close, focus, event, onError, container });

            if (isDefined(decoratedValue)) {
                nextProps[key] = decoratedValue;
            }
        }
    });

    extend(existingProps, nextProps);
}
